...
let slider_about = new Swiper('.slider__body', {
	...
	breakpoints: {
		...
		on: {
		init: function () {
			let r = document.querySelector('.slider__text');
			const activSlide = document.querySelector('.swiper-slide-active');
			const mainText = document.querySelector('.slider__text');
			if (!(activSlide === null)){
				mainText.textContent = activSlide.getAttribute('sas');
...
let redButton = document.getElementsByClassName("left-decision__span");
for (let i = 0; i < redButton.length; i++){
	redButton[i].addEventListener('click', function(e){
		redButton[i].classList.toggle('_active');
		parent = redButton[i].parentElement.querySelector(".left-decision__btn");
		parent.innerHTML = 'Read';
		heightTr =parent.parentNode.parentNode.parentNode .parentNode.querySelector(".left-decision__bottom");
	heightTr.classList.toggle('_active');

		stParent = parent.parentNode.parentNode.parentNode .parentNode.querySelector(".description-left-decision__subtitle");
		stParent.classList.toggle('_active');
		 if(redButton[i].classList.contains('_active') === false){
			 parent.innerHTML = 'Unread'
		 
		//stParent.style.height = 100 + '%';	
	 }
...
let casperBtn = document.querySelector(".top-left-thread__btn");
let showAll = document.querySelector(".top-left-thread__link");
let SpanRed = document.querySelector(".top-left-thread__red");
ifActive = 0;
casperBtn.addEventListener('click', function(e){
	nodDecision = document.getElementsByClassName("decision");
	for (let a = 0; a < nodDecision.length; a++ ){
		nodDecision[a].classList.toggle('_active');
		 if ( nodDecision[a].classList.contains('_active') === true){
			 nodDecision[a].style.transitionDuration = 0.3 + "s";
		 	showAll.innerHTML = 'Show all';
			showAll.style.transitionDuration = 0.3 + "s";	
			SpanRed.style.background= "green";
			SpanRed.style.transitionDuration = 0.3 + "s";		

		}else{
			showAll.innerHTML = 'Dismiss all'
			SpanRed.style.background = "red"
